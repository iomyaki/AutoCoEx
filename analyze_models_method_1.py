...
def analyze_models_method_1(device,
    ...
    def iterate_model_set(device,
        ...
        for model_name, bottleneck_size in iterator:
            ...
            # save the prediction
            adjacency_matrix = pd.DataFrame(np.array(output.detach().cpu()),
            ...
            adjacency_matrix_index_filtered = adjacency_matrix[adjacency_matrix.index.isin(selected_parameters)]
            adjacency_matrix_column_filtered = adjacency_matrix_T[adjacency_matrix_T.index.isin(selected_parameters)]

            # concatenate the adjacency matrix with the list of anomalies
            result_index_filtered = pd.concat([pvalues_sorted_short, adjacency_matrix_index_filtered], axis=1)
